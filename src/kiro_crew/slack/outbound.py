...
def render_for_slack(text: str) -> tuple[str, list[str]]:
    ...
    ANSI escapes are stripped BEFORE step 1, and that ordering protects the
    **choices** specifically. The body is redacted a second time in step 3, after
    ``to_slack_mrkdwn`` has removed the escapes, so a credential the escapes had
    broken up is caught on that pass. The choices get no such second pass — they
    are split off in step 2 and go straight into Block Kit labels and values — so
    if the escapes were still present for step 1, a split credential inside a
    choice would match nothing and reach Slack intact.
    """
    body = _redacted(strip_ansi(text))
